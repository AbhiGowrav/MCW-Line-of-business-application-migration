...
1. Go to the **Azure Migrate** blade in the Azure portal. Select the **Get Started** panel, then select **Assess and migrate databases**.
   ...
   ![Screenshot showing the Azure Migrate Overview blade in the Azure portal, with the 'Assess and migrate databases' button highlighted.](images/Exercise2/EX2-T1-S1.png "Assess and migrate databases button")  
...
3. Select **Azure Migrate: Database Assessment (1)**, then select **Add tool (2)**
...
5. Select **Azure Migrate: Database Migration (1)**, then select **Add tool (2)**.
...
9. In the Data Migration Assistant, select the **+ New** icon.  Fill in the project details as follows:
   ...
   - Project type: **Assessment (1)**
  
   - Project name: **SmartHotelAssessment (2)**
  
   - Assessment type: **Database Engine (3)**
  
   - Source server type: **SQL Server (4)**
  
   - Target server type: **Azure SQL Database (5)**
     
10. Select **Create (6)** to create the project.

    ![Screenshot showing the new DMA project creation dialog.](images/Exercise2/EX2-T1-S10.png "New DMA assessment")

11. On the **Options** tab **select Next**.

12. On the **Select sources** page, in the **Connect to a server** dialog box, provide the connection details to the SQL Server, and then select **Connect (6)**.

    - Server name: **192.168.0.6 (1)**
  
    - Authentication type: **SQL Server Authentication (2)**
  
    - Username: **sa (3)**
  
    - Password: **<inject key="SmartHotelHost Admin Password" /> (4)**
  
    - Encrypt connection: **Checked (5)**
  
    - Trust server certificate: **Checked (5)**

    ![Screenshot showing the DMA connect to a server dialog.](images/Exercise2/EX2-T1-S12.png "Connect to server")

13. In the **Add sources** dialog box, select **SmartHotel.Registration (1)**, then select **Add (2)**.

    ![Screenshot of the DMA showing the 'Add sources' dialog.](images/Exercise2/EX2-T1-S13.png "Add sources")
...
17. On the **Connect to Azure** blade, select **Azure** from the dropdown on the right then select **Connect**. Enter the below Azure credentials when prompted. Select your **Subscription (1)** and **Azure Migrate Project (2)** using the dropdowns, then select **Upload (3)**. Once the upload is complete, select **OK** to dismiss the notification.
...
1. In the Azure portal, expand the portal's left navigation by selecting **Show portal menu** in the top left, then click on **Resource groups (1)** and select the **SmartHotelRG (2)** resource group, select the database server **smarthoteldb<inject key="DeploymentID" enableCopy="false" /> (3)**.
...
2. From the left hand side menu, under **Security** select **Networking (1)** then **Private access (2)** tab. Now, under **Private endpoint connections** click on **+ Create a Private endpoint (3)**.

    ![Azure portal screenshot showing the select path to create a SQL Database.](images/Exercise2/db2.1.png "New SQL Database")

3. On the **Basics** tab that appears, enter the following configuration then select **Next: Resource (5)**. 

   - Resource group: **SmartHotelRG (1)**
  
   - Name: **SmartHotel-DB-for-DMS (2)**

   - Observe **Network Interface name (3)**
  
   - Region: Select **<inject key="Region" enableCopy="false" /> (4)** from the dropdown.
  
   ![Screenshot showing the 'Create a private endpoint' blade, 'Basics' tab.](images/Exercise2/EX2-T2-S3.png "Private Endpoint - Basics")

4. On the **Resource** tab, make sure the following configurations are selected and click on **Next: Virtual Network (5)**. 
  
   - Subscription: **your subscription (1)**.
  
   - Resource type: **Microsoft.Sql/servers (2)**
  
   - Resource: SQL database server **smarthoteldb<inject key="DeploymentID" enableCopy="false" /> (3)** which you created previously.
   
   - Target sub-resource: **sqlServer (4)**

   ![Screenshot showing the 'Create a private endpoint' blade, 'Resource' tab.](images/Exercise2/EX2-T2-S4.png "Private Endpoint - Resource")
...
8. On the **SmartHotel-DB-for-DMS** private endpoint blade, from the left hand side menu select **DNS configuration (1)** which is under **Settings**.
...
9. On the **SmartHotel-DB-for-DMS | DNS configuration**, select the **Private DNS Zone** **privatelink.database.windows.net (2)**.

   ![Screenshot showing step 2 to find the DNS entry for the SQL database server private endpoint](images/Exercise2/EX2-T2-S9.png "Private DNS integration")

10. On the **privatelink.database.windows.net** overview blade, note that the **Record set** with name **smarthoteldb<inject key="DeploymentID" enableCopy="false" /> (3)** and an internal IP address **10.1.0.X** is listed.

      ![Screenshot showing step 3 to find the DNS entry for the SQL database server private endpoint](images/Exercise2/EX2-T2-S10.png "Private Endpoint IP address")
...
11. Return to the Database server blade. Under **Security**, select **Networking (1)** then **Public access (2)** tab. Now, set `Public network access` to **Disabled (3)**, then **Save (4)** your changes.
...
12. Navigate to the Database Migration Service **SmartHotelDBMigration** resource blade in the **AzureMigrateRG** resource group and select **+ New Migration Project**.
    ...
    ![Screenshot showing the Database Migration Service blade in the Azure portal, with the 'New Migration Project' button highlighted.](images/Exercise2/EX2-T2-S12.png "New DMS migration project")
 
13. In the **New migration project** blade, enter the below information,

    - Project name : Enter **DBMigrate (1)** 
    - Leave the source server type as **SQL Server (2)** and target server type as **Azure SQL Database (3)**.
    - Choose type of activity : Select **Create project only (4)** from the dropdown
    - Select **Create (5)**.
 
    ![Screenshot showing the Database Migration Service blade in the Azure portal, with the 'New Migration Project' button highlighted.](images/Exercise2/EX2-T2-S13.png "DMS migration project - settings")
...
14. The Migration Wizard opens, showing the **Select source** step. Complete the settings as follows, then select **Next: Select databases**.
    ...
    ![Screenshot showing the 'Select source' step of the DMS Migration Wizard.](images/Exercise2/EX2-T2-S14.png "DMS project - Select source")
...
15. In the **Select databases** step, the **Smarthotel.Registration** database should already be selected. Select **Next: Select target**.
    ...
    ![Screenshot showing the 'Select databases' step of the DMS Migration Wizard.](images/Exercise2/EX2-T2-S15.png "DMS project - Select databases")
...
15. Complete the **Select target** step as follows, then select **Next: Summary**:
    ...
    ![Screenshot showing the DMS migration target settings.](images/Exercise2/EX2-T2-S16.png "DMS project - select target")

    > **Note**: You can find the target server name in the Azure portal by browsing to your database.

    ![Screenshot showing the Azure SQL Database server name.](images/Exercise2/EX2-T2-S16-1.png "SQL database server name")

17. At the **Project summary** step, review the settings and select **Save project** to create the migration project.

    ![Screenshot showing the DMS project summary.](images/Exercise2/EX2-T2-S17.png "DMS project - summary")
...
1. The Azure portal should show a blade for the DBMigrate DMS project. Select **+ New Activity (1)** and select **Schema only migration (2)** from the drop-down.
...
2. The Migration Wizard is shown. Most settings are already populated from the existing migration project. At the **Select source** step, re-enter the source database password **<inject key="SmartHotelHost Admin Password" />**, then select **Next: Select target**.
   ...
   ![Screenshot showing the 'Select source' step of the DMS Migration Wizard. The source database password is highlighted.](images/Exercise2/EX2-T3-S2.png "Select source")
...
3. At the **Select target** step, enter the password **<inject key="SmartHotelHost Admin Password" />** and select **Next: Select database and schema**.
   ...
   ![Screenshot showing the 'Select target' step of the DMS Migration Wizard. The target database password is highlighted.](images/Exercise2/EX2-T3-S3.png "Select target")
...
4. At the **Select database and schema** step, select the below information,
   ...
   ![Screenshot showing the 'Select database and schema' step of the DMS Migration Wizard.](images/Exercise2/EX2-T3-S4.png "Select database and schema")
...
5. At the **Summary** step, enter the below value as the **Activity name**. Select **Start migration** to start the schema migration process.
   ...
   ![Screenshot showing the 'Summary' step of the DMS Migration Wizard. The activity name, validation option, and 'Run migration' button are highlighted](images/Exercise2/EX2-T3-S5.png "Schema migration summary")

6. The schema migration will begin. Select the **Refresh** button and watch the migration progress, until it shows as **Completed**.

   ![Screenshot showing the SchemaMigration progress blade. The status is 'Completed'.](images/Exercise2/EX2-T3-S6.png "Schema migration completed")
...
1. Return to the Azure portal blade for your **DBMigrate** migration project in DMS. Select **+ New Activity (1)** and select **Offline data migration (2)** from the drop-down.

   ![Screenshot showing the 'New Activity' button within an Azure Database Migration Service project, with 'Offline data migration' selected from the drop-down.](images/a1.2.png "New Activity - Offline data migration")
...
8. The data migration will begin. Select the **Refresh** button and watch the migration progress, until it shows as **Completed**.
   ...
   ![Screenshot from DMS showing the data migration in completed.](images/Exercise2/EX2-T4-S8.png "Data migration completed")
